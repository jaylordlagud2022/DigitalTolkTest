...
class BookingRepository extends BaseRepository
{
     ...
     *
     * @return array Array containing emergencyJobs, normalJobs, cuser, and usertype
     */
    public function getUsersJobs($user_id)
    {
        ...
        if ($cuser) {
            ...
            if ($cuser->is('customer')) {
                // Get jobs for customer with specific relationships and conditions
                $jobs = $cuser->jobs()
                ...
                // Get jobs for translator with specific conditions
                $jobs = Job::getTranslatorJobs($cuser->id, 'new')->pluck('jobs')->flatten();
                $usertype = 'translator';
            }

            // Process jobs if not empty
            if ($jobs->isNotEmpty()) {
                $jobs->each(function ($jobItem) use (&$emergencyJobs, &$normalJobs, $user_id) {
                    // Determine target array based on the 'immediate' property
                    $targetArray = ($jobItem->immediate == 'yes') ? $emergencyJobs : $normalJobs;
                    
                    // Add job item to the target array
                    $targetArray[] = $jobItem->toArray();
                    end($targetArray);
                    
                    // Get the key of the last added item in the target array
                    $itemKey = key($targetArray);
                    
                    // Add 'usercheck' property based on another function
                    $targetArray[$itemKey]['usercheck'] = Job::checkParticularJob($user_id, $jobItem);
                });

                // Sort normal jobs by 'due' property
                $normalJobs = collect($normalJobs)->sortBy('due')->values()->all();
            }
        }
    ...
    /**
     * Get historical jobs for a user based on user type.
     *
     * @param int $user_id User ID
     * @param Request $request HTTP request
     *
     * @return array|null Array containing historical job information or null if user not found
     */
    public function getUsersJobsHistory($user_id, Request $request)
    {
        // Get the requested page from the request or default to page 1
        ...
        if ($cuser) {
            // Check if the user is a customer
            if ($cuser->is('customer')) {
                // Get historical jobs for a customer with specific relationships and conditions
                $jobs = $cuser->jobs()
                ...
                // Return an array containing historical job information for the customer
                return [
                ...
                // Get historical jobs for a translator with specific conditions
                ...
                // Set user type to 'translator'
                $usertype = 'translator';

                // Return an array containing historical job information for the translator
                return [
                    'emergencyJobs' => $emergencyJobs,
                    'normalJobs' => $jobs,
                    'jobs' => $jobs,
    ...
    /**
     * Store a new job for a user.
     *
     * @param User $user The user for whom the job is created
     * @param array $data The data for creating the job
     *
     * @return array Response status and additional information
     */
    public function store(User $user, array $data)
    {
        ...
        if ($user->user_type == env('CUSTOMER_ROLE_ID')) {
            ...
            $validationErrors = $this->validateCustomerFields($data);
            if (!empty($validationErrors)) {
                return $validationErrors;
            }

            // Set default values for phone and physical type
            ...
            // Set due date and time based on immediate or regular booking
            ...
            $this->setGenderAndCertifiedFields($data);

            // Set job type based on consumer type
            $this->setJobType($data, $consumerType);

            // Set additional fields and create the job
            $this->setAdditionalFieldsAndCreateJob($data, $cuser);
            ...
            // Event and notification handling (commented out for now)
     ...
     */
    private function validateCustomerFields(array $data)
    {
        $requiredFields = ['from_language_id', 'duration', 'due_date', 'due_time', 'customer_phone_type', 'customer_physical_type'];
        $validationErrors = [];

        foreach ($requiredFields as $field) {
            if (!isset($data[$field]) || empty($data[$field])) {
                $validationErrors['status'] = 'fail';
                $validationErrors['message'] = "Du måste fylla in alla fält";
                $validationErrors['field_name'] = $field;
                return $validationErrors;
            }
        }
     ...
     * @param array $data The data for creating the job
     */
    private function setGenderAndCertifiedFields(array &$data)
    {
     ...
     *
     * @param array $data The data for creating the job
     * @param string $consumerType The consumer type of the user
     */
     ...
     *
     * @param array $data The data for creating the job
     * @param User $cuser The customer user
     */
    private function setAdditionalFieldsAndCreateJob(array &$data, User $cuser)
    {
    ...
    /**
     * Get the job_for array based on job gender and certified fields.
     *
     * @param Job $job The created job
     *
     * @return array The job_for array
     */
    ...
    /**
     * Get all jobs based on the request filters.
     *
    ...
    public function getAll(Request $request, $limit = null)
    {
        ...
        // Get the authenticated user and their consumer type
        ...
        // Apply filters based on the user type
        if ($cuser && $cuser->user_type == env('SUPERADMIN_ROLE_ID')) {
            $allJobs = $this->applySuperAdminFilters($allJobs, $requestdata);
        } else {
            $allJobs = $this->applyConsumerFilters($allJobs, $requestdata, $consumer_type);
        }

        // Apply common order and pagination logic
        $allJobs = $this->applyOrderAndPagination($allJobs, $limit);

        return $allJobs;
    }

    /**
     * Apply filters specific to super admin users.
     *
     * @param \Illuminate\Database\Eloquent\Builder $allJobs
    ...
    /**
     * Apply filters specific to consumer users.
     *
     * @param \Illuminate\Database\Eloquent\Builder $allJobs
     * @param array $requestdata
     ...
     * @return \Illuminate\Contracts\Pagination\LengthAwarePaginator|\Illuminate\Database\Eloquent\Collection
     */
    private function applyOrderAndPagination($allJobs, $limit)
    {
        ...
        // Return the result based on the limit parameter
        if ($limit == 'all') {
            return $allJobs->get();
        } else {
    ...
    /**
     * Store job-related email information and send confirmation email.
     *
     * @param array $data
     * @return array
     */
    public function storeJobEmail($data)
    {
        ...
        // Find the job by ID or throw an exception if not found
        $job = Job::findOrFail(@$data['user_email_job_id']);

        // Update job information from the provided data
        ...
        // Update additional job information based on the provided data
        ...
        // Determine email and name for sending confirmation email
        ...
        // Trigger event for job creation
    ...
    /**
     * Convert job information to a suitable format for sending Push notifications.
     *
    ...
    {
        $data = array();  // Initialize an array to store job information for Push notification

        // Extract and store essential job details
        $data['job_id'] = $job->id;
        ...
        // Extract and format due date and time
        $dueDateParts = explode(" ", $job->due);
        $data['due_date'] = $dueDateParts[0];
        $data['due_time'] = $dueDateParts[1];

        $data['job_for'] = array();  // Initialize an array to store job-related details

        // Add job "for" details based on gender
        if ($job->gender != null) {
            if ($job->gender == 'male') {
                $data['job_for'][] = 'Man';
            } elseif ($job->gender == 'female') {
        ...
        // Add job "for" details based on certification
        if ($job->certified != null) {
            if ($job->certified == 'both') {
        ...
        return $data;  // Return the formatted job data
    }
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    public function updateJob($id, $data, $cuser)
    {
        ...
        // Retrieve the current translator for the job
        $current_translator = $job->translatorJobRel->where('cancel_at', null)->first();

        // If no current translator without cancellation is found, use the one with completed_at not null
        ...
        // Flag to check if language is changed
        $langChanged = false;

        // Check and log if translator is changed
        ...
        // Check and log if due date is changed
        ...
        // Check and log if the language is changed
        ...
        // Check and log if the status is changed
...
/**
 * Accept a job and send confirmation emails.
 *
 * @param array $data
 * @param User $user
 * @return array
...
public function acceptJob($data, $user)
{
    ...
    if (!Job::isTranslatorAlreadyBooked($job_id, $cuser->id, $job->due)) {
        ...
        if ($job->status == 'pending' && Job::insertTranslatorJobRel($cuser->id, $job_id)) {
            ...
            // Data to be sent with the confirmation email
            $data = [
                'user' => $user,
                'job'  => $job
            ];
        ...
        // Get potential jobs for the current user
        ...
        // Provide an error message if the translator is already booked for the job
        $response = [
            'status'  => 'fail',
    ...
    /**
     * Accept a job with the given ID for the specified user.
     *
     * @param int $job_id
     * @param User $cuser
     * @return array
    ...
    public function acceptJobWithId($job_id, $cuser)
    {
        ...
        if (!Job::isTranslatorAlreadyBooked($job_id, $cuser->id, $job->due)) {
            ...
            if ($job->status == 'pending' && Job::insertTranslatorJobRel($cuser->id, $job_id)) {
                ...
                // Notification data for push notification
                $notificationData = [
                    'notification_type' => 'job_accepted',
                ];

                // Message text for the push notification
                $msg_text = [
                ...
                if ($this->isNeedToSendPush($user->id)) {
                    ...
                    $this->sendPushNotificationToSpecificUsers($users_array, $job_id, $notificationData, $msg_text, $this->isNeedToDelayPush($user->id));
                }
